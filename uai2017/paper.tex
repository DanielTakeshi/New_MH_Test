...
675008). \textbf{Daniel: Xinlei, these numbers don't match
Figure~\ref{fig:gauss_mix_2} and the figures need to be updated so the scatter
plot has the correct method names and the scaling key, while the histograms have
the correct names and the seaborn background.} We
further ran 10 runs of mixture of Gaussians experiments, and the average and
standard deviation values of the mean minibatch size for {\sc MHminibatch}, {\sc
AustereMH(c)}, {\sc MHSubLhd} are listed in table ~\ref{tab:gaussianres}. Sizes
correspond to the running times of the methods, excluding the likelihood
computation of all data points for {\sc MHSubLhd}, which would drastically
...
each). \textbf{Daniel: Xinlei, can you add in one more decimal point for all the
numbers in this table, like I do in the Logistic Regression table?}}
...
\begin{table}[t]
\caption{Gaussian Mixture Model Statistics \textbf{TODO Xinlei, please fix this
table.}}
\small
\label{tab:poissons}
\vskip 0.15in
...
Chi-Squared test statistics value as well as their standard deviations.  Our likelihood values 
lies between~\citep{cutting_mh_2014} and~\citep{icml2014c1_bardenet14}, 
but the variance of their likelihood values are high and
ordering changes depending on the range of samples generated. It should be noted 
that we are not aiming at optimizing the likelihood values nor the Chi-Squared statistics.
We use these values to show the extent of correctness. \textbf{Daniel:
Xinlei, one reviewer had a problem with this, and I agree. The easiest fix here
is to simply report standard deviation values. Also, we should emphasize that we
are not aiming to get the best score by this metric but simply something that
