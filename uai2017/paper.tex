...
\begin{algorithm}[tb]
\caption{Our acceptance test, {\sc MCminibatch}.}
...
\textbf{Daniel: TODO this should be where we mention that AustereMH still
requires computation of all likelihoods, and we should explain John's complaint
about Gaussianity here.}
...
In Sections~\ref{ssec:gaussians} and~\ref{ssec:logistic}, we benchmark our test,
{\sc MHminibatch}, against the test from~\cite{icml2014c1_bardenet14} ({\sc
MHSubLhd}) and from~\cite{cutting_mh_2014}. As highlighted in
Section~\ref{sec:analysis}, the test from~\cite{cutting_mh_2014} can be
implemented using a conservative or a non-conservative variant; we name these
{\sc AustereMH(c)}, and {\sc AustereMH(nc)}, respectively, and they use the
grid-search over minibatch sizes $m$ and per-test thresholds $\epsilon$
described in Appendix~\ref{sssec:grid_search}. Throughout our descriptions, we
refer to a \emph{trial} as the period when an algorithm collects all its desired
samples $\{\theta_1,\ldots,\theta_T\}$, with $T$ usually several thousand.
\textbf{Daniel: better call temperature something other than $T$ ...}
...
This model is adapted from~\cite{langevin_2011} by increasing the number of
...
We benchmark with {\sc AustereMH(c)} and {\sc MHSubLhd}. {\sc MHminibatch} and
{\sc MHSubLhd} were initialized with $m=100$. For {\sc MHSubLhd}, we increase
...
covariance ${\rm diag}(0.15, 0.15)$, which means the M-H test is responsible for
shaping the sample distribution.
...
16857). The sizes correspond to the running times of the methods, excluding the
...
\begin{figure*}[t]
    ...
    {\sc MHminibatch} on both datasets when the temperature starts at $T=100$
    and drops to $T=1$ after a ``burn-in'' period of 200 samples of $\theta$.
    For all three plots, one standard deviation is indicated by the shaded error
    regions.
    }
...
\begin{figure*}[t]
	...
	\caption{
    Minibatch sizes for a representative trial of logistic regression on
    MNIST-100k (similar to Figure~\ref{fig:logistic_histograms_mnist}). Both
    axes are on a log scale and have the same ranges across the three
    histograms.  See Section~\ref{ssec:logistic} for details.
    }
...
\begin{table}[t]
\caption{
Average minibatch sizes ($\pm$ one standard deviation) for the methods on the
logistic regression tasks of binary classification on MNIST-13k and MNIST-100k.
The averages are taken over 10 independent trials (5000 samples each) for
MNIST-13k and 5 independent trials (3000 samples each) for MNIST-100k.
}
...
{\sc AustereMH(nc)} & $973.8\pm 49.8$   & N/A \\
...
on the MNIST~\cite{lecun-mnisthandwrittendigit-2010} dataset and (a subset of)
infinite MNIST~\cite{loosli-canu-bottou-2006}. For the former, extracting all 1s
...
MNIST-100k. The default temperature setting is a constant at $T=100$ for
...
will generally result in worse performance for all methods. For MNIST-13k, each
method generated 5000 samples for ten independent trials; due to MNIST-100k's
higher computational requirements, the methods generated 3000 samples for five
...
$C_{\theta,\theta'}$ described in~\cite{icml2014c1_bardenet14}, but it was too
...
$t\in\{1,\ldots,N\}$ ($N=5000$ for MNIST-13k, $N=3000$ for MNIST-100k), we use
$\theta_t$ as the parameter for logistic regression. The results indicate that
our test is more efficient, obtaining convergence more than an order of
magnitude faster than {\sc AustereMH(nc)} and several orders of magnitude
compared to {\sc AustereMH(c)} and {\sc MHSubLhd}. We also observe the advantage
of having higher temperature from the third plot in
...
temperature to 1 (equivalent to ignoring temperature). {\rm MHminibatch}
requires the step size to decrease, hence the smaller changes in accuracy.

Figures~\ref{fig:logistic_histograms_mnist}
and~\ref{fig:logistic_histograms_mnist8m} show histograms of minibatch sizes for
the three methods on a log-log scale for MNIST-13k and MNIST-100k, respectively.
The histograms only represent one representative trial; Table~\ref{tab:logistic}
contains the average of the average minibatch sizes ($\pm$ one standard
deviation) across either 10 or 5 trials. The results indicate that {\sc
MHminibatch}, with average minibatch sizes of roughly 129.8 and 202.5 for
MNIST-13k and MNIST-100k, respectively, consumes almost an order of magnitude
fewer data points than the next-best method, {\sc AustereMH(nc)}.  We reiterate,
however, that both {\sc AustereMH(nc)} and {\sc MHSubLhd} require computing
$\log p(x_i|\theta)$ and $\log p(x_i|\theta')$ for all $x_i$ each iteration.
These results here do not count that extra data consumption. Only our method and
{\sc AustereMH(c)} rely solely on the minibatch of data each iteration.
...
with~\cite{cutting_mh_2014} by applying both tests each iteration or utilizing
the variance reduction techniques suggested in~\cite{DBLP:conf/icml/ChenG16}.
...
(following the framework of~\cite{sgmcmc_2015}).
...
% Daniel: TODO remove for anonymous review later.
...
\begin{center}
\begin{tabular}{l l l l}
\textbf{Metric} & {\sc MCminibatch} & {\sc AustereMH} & {\sc MHSubLhd} \\
\hline \\
...
from~\cite{cutting_mh_2014}, which avoids the need for recomputing log
...
Table~\ref{tab:poissons}). Scores lie between~\cite{cutting_mh_2014}
and~\cite{icml2014c1_bardenet14}, but the variance of these values is high and
...
\begin{figure*}[t]
    ...
    \includegraphics[width=0.75\linewidth]{logistic_regression_histograms_mnist8m.png}
	\caption{
    \textbf{Daniel: TODO this is a wrong figure, I'll add what I want later}
    }
	\label{fig:logistic_histograms_appendix}
\end{figure*}

\begin{table}[t]
\caption{Parameters for the logistic regression experiments. Unless otherwise
stated, values apply to all methods tested. For the methods
from~\cite{cutting_mh_2014}, we use their notation for clarity.}
\small
...
{\sc AustereMH(c)} $\Delta^*$ bound & 0.1 & 0.2 \\
...
{\sc AustereMH(nc)} $\Delta^*$ bound & 0.05 & 0.1 \\
\end{tabular}
\end{center}
\vskip -0.1in
\end{table}

\textbf{Daniel: TODO major work in progress.}

Table~\ref{tab:logistic_settings} indicates the parameter settings for the
logistic regression experiments.

Figure~\ref{fig:logistic_histograms_appendix} indicates minibatch histograms for
one trial of {\sc MHminibatch} on MNIST-100k for proposers $\sigma I$ with
$\sigma \in \{0.005, 0.01, 0.05\}$.
