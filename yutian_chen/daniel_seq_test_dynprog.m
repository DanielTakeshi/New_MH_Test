...
% Daniel: I am modifiying this based on Yutian Chen's code. In particular, we
% will be running this to tabulate the `err` and `mean_j` values. The `mean_j`
% value will be useful to compute \bar{\pi} estimates given the initial batch
% of, say, 100 elements. We can simply do 100 * mean_j. I also have to figure
% out how to run this many times, which I assume will require some external
% matlab calls.
%
% Sequential test. Assume the joint distribution of all the data items is a
% Gaussian distribution.
%
% Use the default value of an input argument by omitting it if no nondefault
% arguments are used after it, or by providing an empty vector [].
%
% Input:
