...
samples to converge to the posterior distribution of a Gaussian mixture model. In
Section~\ref{ssec:logistic}, we analyze the efficiency of our sampler on logistic regression. In
Section~\ref{ssec:nets}, we show that our minibatch MH can obtain good MAP estimates for deep
learning.
...
\begin{align*}
{\rm Var}(\Delta') & = {\rm Var}\left[\sum_{i=1}^m g_i(\theta') - g_i(\theta_t)\right] = {\rm Var}(\sum_i^m q_i) = m {\rm Var} (q_i) + m(m-1) {\rm Cov}_{i \neq j}(q_i,q_j) \\
&= m{\rm Var}(q_i) - \frac{m(m-1)}{N-1} {\rm Var}(q_i) \leq 4(m - \frac{m(m-1)}{N-1})k^2 \epsilon^2, \\
\end{align*}
as desired.
