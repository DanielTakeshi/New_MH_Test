...
and $2N+1$ values respectively. Writing $X_i = i*V/N$ and $Y_j = j*V/N$, the
...
\end{equation}
Defining a matrix $M$ and vectors $u$ and $v$ such that $M_{ij} =
\Phi_{\sigma}(X_i-Y_j)$, $u_j = C_{\sigma}(Y_j)$, and $v_i = S(X_i)$,
...
\caption{A description of M-H sampling with our acceptance test. {\color{blue}
...
not by much, but it still changes sometimes?}}
\label{alg:our_algorithm}
\end{algorithm}

\begin{table}[t]
    \caption{\color{blue} Daniel: TODO need a good title.}
    \label{tab:xcorr}
    \centering
    \begin{tabular}{l l l l}
    \toprule
    $N$ & $\sigma$ & $\lambda$ & $L_1$ error  \\
    \midrule
    4000 & 0.9 & 1 & 1.0e-4  \\
