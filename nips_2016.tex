...
posterior temperature at $T_0=3000$, and decrease it according to $T_i = T_0 /(i + 1)^{3}$ for $i
\leq N/2$ and $T_i = T_0 /(i + 1)^{0.2}$ for $i >N/2$, where $N=4000$ is the total number of
iterations. We set the minibatch size $m=50$ and compare with adaptive sampling.
...
For our minibatch MH test, in order to enforce the ${\rm std}(\Delta') < 1.2$ condition, we
initialize the temperature $T_0 = 3000$, and decrease it by the rule $T_i = T_0 /(i + 1)^{3}$ for $i
\leq N/2$ and $T_i = T_0 /(i + 1)^{0.2}$ for $i >N/2$, where $N=4000$ is the total number of
iterations. If our estimated ${\rm std}(\Delta') \ge 1.2$, we ignore the current iteration.
Figure~\ref{fig:appendixExp2} plots our estimated ${\rm std}(\Delta')$ values versus iteration
count. 
