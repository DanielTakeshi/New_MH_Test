...
zero.  Given our choice of intervals for the variables, the RHS is in the range
$\frac{1}{\sqrt{N}}[-1,0.75]$. The standard deviation of the LHS given the
interval constraints is at least $0.5/\sqrt{b}$.  And so the gap between the LHS
and RHS in Equation~\ref{eq:accept_test_rewritten} is at most $2\sqrt{b/N}$
standard deviations, limiting the range in which the test will be able to
``succeed'' without requiring more samples.
