...
$\Delta'$ is that they use $N$ and $n$ data terms $x_i$, respectively. We assume $n \ll N$ (so
...
\end{equation}
The approximation exists because $X_{\rm norm}$ is an estimate of $\varepsilon$.
...
we know the distribution of $X_{\rm corr}$? We need the correct distribution to sample $X_{\rm
corr}$ appropriately. This procedure consists of two major steps.
...
\begin{enumerate}[noitemsep]
    ...
    usually small, around 5-10, and $n$ is usually a few hundred for the Central Limit Theorem to
    apply.  Using these $K$ values, we use the empirical standard deviation ${\rm std}(\Delta')$.

    \item Once we have ${\rm std}(\Delta')$, we can determine the distribution of
    $X_{\rm corr}$ from standard deconvolution techniques. These rely on the well-known fact that
    the Fourier transform of a convolution is the product of the Fourier transforms. In practice,
    we pre-compute distributions of $X_{\rm corr}$ for different ${\rm std}(\Delta')$ values to
...
\KwResult{A series of sampled $\{\theta_i\}$ under the posterior distribution} 
...
\While{not enough sampled data}{
	...
	choose the distribution of $X_corr$ from the pre-computed distribution family\;
...
\caption{A description of our MH test. {\color{blue} TODO}}
...
$\Delta' = \Delta + X_{\rm norm} + X_\xi$. We assume the standard derivation of $X_{\rm norm}$ is less than $\pi/\sqrt{3}$, so that we can always apply deconvolution process in Eq~\cite{eq:deconvolution} without using a temperature. We also denote $F_X$ as the CDF of $X$.  Let the
