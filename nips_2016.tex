...
which means that we can find a $u \in [0,1]$ so that $\psi(u,\theta,\theta') =
\log u < 0$ equals $E[\Lambda^*(\theta,\theta')]$.  Specifically, choose $u_0$
to satisfy $\log u_0 = E[\Lambda^*(\theta,\theta')]$.  Using $E[x_i] = 0.5$, we
see that
\begin{align}
...
\psi(u,\theta,\theta')$, where $\not\approx$ informally means ``significantly
different from'' under the distribution over samples of $x_i$. This turns out to
be
\begin{align}
...
{\color{blue} Daniel: TODO I have not yet updated the rest of this section.}
