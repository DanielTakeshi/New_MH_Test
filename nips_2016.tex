...
balance. To reconcile these competing objectives,~\cite{cutting_mh_2014} propose an adaptive
...
\begin{figure}[t]
  ...
  \caption{
  {\color{blue}
  Daniel: this image is finalized, but change this caption.
  }
  }
...
acceptance criteria is equivalently expressed as $\Delta + X>0$.
...
\end{equation}
This deconvolution process is visualized in Figure~\ref{fig:deconvolution}. We decompose $X$ this
way so that we can estimate $X_{\rm norm}$ from the data, and then add the correction $X_{\rm corr}$
later.  Notice that we have some control over the true value of $X_{\rm norm}$, even though we
cannot generally guarantee computing it exactly. In particular, increasing the minibatch size will
cause the $X_{\rm norm}$ distribution to become narrower. {\color{blue} (Daniel: note that we will
be fixing this figure, so the visualization should hopefully be a little clearer, along with the
...
{\color{blue}
Daniel: we really need to make this clear and do the algorithm description.
}

\begin{algorithm}[t]
\KwData{$\{x_i\}$ under a target distribution}
\KwResult{a serise of sampled $\{\theta_i\}$ under the posterior distribution} 
set the $var(\Delta') = 1.0$ and deconvolve $X$ to get the disribution of $X_{corr}$\;
\While{not enough sampled data}{
	estimate the $sd(\Delta')$\;
    propose a candidate $\theta'$\;
    compute the noisy $\Delta'$\;
    sample a $x$ from $X_{corr}$\;
    \eIf{$x*sd(\Delta') + \Delta' > 0$}{
        Accept the $\theta'$;
    }{
        Reject the $\theta'$;
    }
...
Similarly, we use random walk as the proposer. We compare our mini-batch MH testing with the
...
In this experiment, we also illustrate that by applying the temperature, we can achieve slight
faster convergence than the constant temperature one.
