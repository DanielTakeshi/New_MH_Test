...
\Delta = \log \left( \frac{p(\theta') \left ( \prod_{i=1}^N p(x_i \mid \theta') \right)^t q(\theta_t \mid
\theta')}{p(\theta_t)\left (\prod_{i=1}^N p(x_i \mid \theta_t) \right) ^t q(\theta' \mid
\theta_t)} \right);\;  \Delta' = \log \left( \frac{p(\theta') \left ( \prod_{i=1}^n p(x_i \mid \theta') \right ) ^t q(\theta_t \mid
\theta')}{p(\theta_t) \left ( \prod_{i=1}^n p(x_i \mid \theta_t) \right ) ^t q(\theta' \mid
\theta_t)} \right)
\end{equation}
...
\frac{1}{t}\sum_{i=1}^n (\log p(x_i\mid \theta') - \log p(x_i\mid \theta_t)).
\]
If we fix $\theta_t$ and $\theta'$, the randomness in our $\Delta'$ samples only comes from the
minibatch. Since the minibatch is chosen randomly, the distribution of $\frac{1}{t}\sum_{i=1}^n (\log p(x_i\mid
...
(\log \Pr(x_i| \theta))| < k$, the variance of $\Delta'$ is bounded by $4\epsilon^2 k^2 m /t^2$, where
$m$ is the minibatch size, and $t$ is the temperature.
\end{lemma}

\begin{proof}
From Lemma~\ref{lem:gaussian}, only the $\frac{1}{t}\sum_{i=1}^n (\log p(x_i\mid \theta') - \log p(x_i\mid
...
\[
{\rm Var}(\Delta') = {\rm Var}\left[\frac{1}{t}\sum_{i=1}^n g_i(\theta') - g_i(\theta_t)\right] \leq
\sum_{i=1}^n \frac{1}{t^2} {\rm Var}(g_i(\theta') - g_i(\theta_t)) \leq  4\epsilon^2 m k^2 /t^2,
\]
as desired.
\end{proof}

Since ${\rm Var}(\Delta') < \epsilon^2 k^2 m / t^2$, we can assume the maximum value of $X_{\xi}$ is
proportional to the standard deviation of $\Delta$, i.e. $\epsilon / t$. Thus, we can assume
$\zeta=\epsilon C/t$, where $C$ is a constant factor.
...
where $\alpha_i = \frac{\epsilon_i C }{t_i(1-\eta)}$.
\end{theorem}

\begin{proof}
First, by Theorem 1 in~\cite{cutting_mh_2014}, $\|T \circ P_0 - \pi\| \leq \eta \|P_0
- \pi\|$ and $\|T-T'_i\| \leq \epsilon_i C/t_i$, the norm between the stationary distribution is $\|\pi
- \pi_i'\|\leq \frac{\epsilon_i C}{t_i(1-\eta)}$.
...
\alpha_s$, where $\alpha = \frac{\epsilon_i C}{t_i(1-\eta)}$. Thus, we get:
\begin{align*}
...
The Theorem above indicates that the difference of the distburition obtained by applying the 
...
339 method using a random walk proposer q(θ|θt) = N (θt, σ2 ). Since the random walk proposer does RW
