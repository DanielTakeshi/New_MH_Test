...
over the dataset, and therefore have high sample efficiency. They also
map very well onto hardware such as GPUs. In contrast, MCMC methods
typically require calculations over the full dataset to produce a new
sample.  Recent results
from~\cite{cutting_mh_2014,icml2014c1_bardenet14} perform approximate
(bounded error) acceptance tests based on the classical M-H test. The
tests depend on minibatch statistics, and on the value of an
additional random variable. The amount of data consumed for each test
varies significantly from one minibatch to the next, and
non-repeatably on the random variable. By contrast,
...
(relative) variance of the log acceptance probability ratio. This variance
changes slowly on typical problems, and we can estimate it before
performing the test.

If the variance test fails, there are three
...
\item For continuous probability distributions, reduce the proposal step
  size. Since the condition is on variance in the {\em difference} in
  log probabilities of prior and proposed likelihoods, smaller proposal steps
  will lead to smaller changes in probability and smaller changes in the
  variance of those changes.
\end{itemize}

Note that changes to the distribution temperature may only need to be
temporary.  The distribution can be annealed from high-to-low
temperatures to maintain the variance condition. High temperature is
natural with small minibatches and large step sizes, since the
precision of the acceptance test is limited by the amount of data
available. As the distribution reaches a stationary state and is cooled,
particle motion can be slowed with larger minibatches or smaller proposal
steps. The contributions of this paper are as follows:

\begin{itemize}
\item We developed a new minibatch acceptance test which satisfies detailed balance.
\item we developed effectively measurable bounds for both absolute and relative
  test error. 
\item We compared performance of our new test and prior approaches. 
\item We did experiments using the approach for posterior estimation.
\end{itemize}
...
In standard MCMC methods~\cite{gilks1996markov,brooks2011handbook}, a difficult-to-compute
...
\alpha(\theta',\theta_t) = \min\left\{ 1, \frac{p(\theta')q(\theta_t \mid \theta')}{p(\theta_t)q(\theta' \mid \theta_t)}
\right\}
\end{equation}
and with probability $\alpha(\theta',\theta_t)$, we accept $\theta'$
and set $\theta_{t+1} = \theta'$, otherwise set
$\theta_{t+1}=\theta_t$.  The test is often implemented with an
auxiliary random variable $u \sim \mathcal{U}(0,1)$ with a comparison $u < \alpha(\theta',\theta_t)$


The acceptance test guarantees detailed balance, so that
\begin{equation}
...
\begin{equation}
\alpha(\theta',\theta_t) = 
\min\left\{ 1, \frac{p(\theta')\prod_{i=1}^N p(x_i \mid \theta')q(\theta_t \mid
\theta')}{p(\theta_t)\prod_{i=1}^N p(x_i \mid \theta_t)q(\theta' \mid\theta_t)} \right\},
\end{equation}
where $p(\theta)$ is a prior, and $p(x_i \mid \theta)$ are the
probabilities of the observations. Computing samples this way requires
the use of all $N$ training data points. Moreover, it is difficult to
design tests using substantially fewer than $N$ points that also
satisfy detailed balance. To reconcile these competing
objectives,~\cite{cutting_mh_2014,icml2014c1_bardenet14} perform
...
approximate version of the test $u < \alpha(\theta',\theta_t)$. If the
...
worst case can occur about $\sqrt{N}$ times during the performance of
$N$ tests.

Following ~\cite{icml2014c1_bardenet14}, we write the test $u < \alpha(\theta',\theta_t)$ in
...
By the central limit theorem, we expect $\Lambda(\theta_t,\theta')$ to be approximately
Normal. To reduce computational effort, an unbiased estimate of $\Lambda(\theta,\theta')$
...
bound of average number of data instance consumed for one iteration of
~\cite{cutting_mh_2014} and ~\cite{icml2014c1_bardenet14} is
$\Omega(\sqrt{N})$, where $N$ is the number of data points.
...
\begin{equation}
\Lambda^*(\theta,\theta') =  \frac{1}{b}\sum_{i=1}^b \log\frac{p(x_i|\theta')}{p(x_i|\theta_t)}=
  %%\frac{1}{2}\left( (x_i-\theta')^2 - (x_i - \theta_t)^2\right) =
  (\theta'-\theta_t)\left(\frac{1}{b}\sum_{i=1}^b x_i-\theta_t-\frac{\theta'-\theta_t}{2}\right)
\end{equation}
...
of $\Lambda^*(\theta,\theta')$ is $(\theta'-\theta_t)^2/b$. In order to pass
a hypothesis test that $\Lambda > \psi$, we will need to establish a
large enough gap (several $\sigma(\Lambda^*)$) between $\Lambda^*(\theta',\theta_t)$
and $\psi(u,\theta',\theta_t)$.
...
$\mathcal{N}(\mu,1/N)$, so we take $q(\theta'\mid\theta_t) =
\phi((\theta'-\theta_t)N)$, where $\phi(x)$ is a Normal density
function with zero mean and unit variance. This proposal is symmetric
$q(\theta',\theta_t)=q(\theta_t,\theta')$, and we assumed uniform
prior for $\theta$, so $\psi(u,\theta',\theta_t)$ reduces to $1/N\log
u$.

\begin{lemma}\label{lem:efficienty_cutting}
  For the model above, there
  exists a fixed constant $c$ such that with probability $ \geq c$ over the joint
  distribution of $(\theta_t, \theta', u)$, the
  test from \cite{icml2014c1_bardenet14} consumes all $N$ samples. 
\end{lemma}

\begin{proof} Assume $(\theta' - \theta_t) \in \pm\frac{1}{\sqrt{N}}[0.5,1]$ and
  $\theta_t -0.5 \in \pm\frac{1}{\sqrt{N}}[0.5,1]$ with matching sign. These events
  occur with probability $p_0=2*(\Phi(1)-\Phi(0.5)) = 0.2997$ and $p_1=(\Phi(1)-\Phi(0.5))=0.14988$ respectively. 
  Choose $u_0$
  to satisfy
  \begin{equation}
    \log u_0 = -N(\theta'-\theta_t)\left(\theta_t-0.5+\frac{\theta'-\theta_t}{2}\right) 
  \end{equation}
  and rewrite the test now as
  \begin{equation}
\frac{1}{b}\sum_{i=1}^b x_i-\left(\theta_t+\frac{\theta'-\theta_t}{2} + \frac{\log u_0}{N(\theta'-\theta)}\right) > 0
  \end{equation}
 ...
 The choices above for $\theta_t$ and $\theta'$ guarantee that
  \begin{equation}
    \log u_0 \in -[0.5,1][0.75,1.5] = -[0.375,1.5]
        \end{equation}
and consider the range of $u$ values
  \begin{equation}
    \log u \in \log u_0 + [-1,0.375]
  \end{equation}
  the size of this range is at least $\exp([-2.5,-1.125]) = [0082,0.286]$ and occurs
  with probability at least $p_2=0.2044$. The samples $\theta_t$, $(\theta'-\theta_t)$
  and $u$ are drawn independently and so the total probability of these events is
  $c = p_0 p_1 p_2 = 0.009$.
\end{proof}
...
\For{$t=\{1, \ldots, T\}$}{
    ...
    Draw a minibatch of $m$ samples, compute $\Delta'(\theta',\theta_t)$ and ${\rm std}(\Delta')$ for this sample\;
