...
\begin{algorithm}[t]
\Input{Samples $T$, number of $\Delta'$ estimates $K$, minibatch size $m$, pre-computed $X_{\rm corr}$ values.}
\Output{A chain of $T$ samples $\{\theta_1, \ldots, \theta_T\}$ for either posterior inference or point estimation.}
Initialize $\theta_1$\;
\For{$t=\{2, \ldots, T\}$}{
    Create a new empty list for $\Delta'$ estimates, $d = []$\;
    \For{$k = \{1, \ldots, K\}$}{
    ...
    Compute ${\rm std}(d)$, the standard deviation of the $\Delta'$ elements from list $d$\;
    Using a random minibatch of $m$ data points, compute a \emph{new} $\Delta'$; call it $\Delta_{\rm real}'$\;
    Sample $u \sim {\rm Unif}[0,1]$ and extract the corresponding $X_{\rm corr}$ from pre-computed data\;
    Re-scale the correction error: $X_{\rm corr} = X_{\rm corr} \cdot {\rm std}(d)$\;
    {\color{blue} Daniel: TODO, check if we need some variance correction or something here.}\;
    \eIf{$X_{\rm corr} + \Delta_{\rm real}' > 0$}{
