...
perform Metropolis-Hastings tests on an entire dataset. Instead we
...
batch sizes, which may use far more than one minibatch of data and/or
yield few samples.  Here we describe a simpler test on fixed-sized
minibatches. Instead of adaptive batch size, this test requires a
precondition on the variance of the log proposal probability
ratio. Given a measurement of this variance, minibatch size can be
chosen to support the test in one step, and yielding a number of
...
particular, are based on minibatch updates (such as SGD) to a model.may
Minibatch updates produce many improvements in the model for each pass
over the dataset, and have high sample efficiency. They also map very
well onto hardware such as GPUs. MCMC methods in contrast, typically
...
Here we develop an alternative minibatch acceptance test for MCMC
methods which uses only one fixed-size minibatch per test. Instead of
dynamically adjusting the batch size, we add a prequisite for the test
...
before performaning the accpeptance test. The minibatch variance
descreases with the minibatch size, so the variance measurement can be
used to set the minimum minibatch size.

There is an additional advantage when using the test for Bayesian
posterior inference. As dataset size grows, the posterior distribution
sharpens and will often concentrate to a few sharp peaks. Samples from
this distribution will come from those peaks, and will easily become
stuck in one of them (see section 5.1).  The samples are not exploring
...
sample of the posterior because of inadequate search (inadequate
mixing). We argue that it is much more natural to sample the
distribution at a different (higher) temperature determined by the
...
Although this approach also involves changing minibath size, the
...
\begin{enumerate}[noitemsep]
    ...
    \item Experiments using the approach for posterior estimation.
\end{enumerate}
