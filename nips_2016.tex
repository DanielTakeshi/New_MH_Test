...
\end{equation}
where we assume $X_{\rm norm}$ is a zero-mean Gaussian and $X_{\rm
  corr}$ is a zero-mean ``correction'' term with distribution $C(X)$.
The two variables are added (i.e., their distributions convolve) to
form $X_{\rm log}$. It will be convenient to express this relationship
in terms of the CDFs $N(X_{\rm norm}) = \Phi(X_{\rm norm}/\sigma)$ and
$S(X_{\rm log})$, where $\Phi(.)$ is the Normal CDF with zero mean and
unit variance and $S(.)$ is the standard logistic function.
 
Equation (\ref{eq:deconvolution}) implies that $S = N * C$ where $*$ denotes convolution. We will later
...
corr}$ distribution, and initial sample $\theta_1$.}
...
\For{$t=\{1, \ldots, T\}$}{
    ...
    Draw a minibatch of $m$ points $x_i$, compute $\Delta^*(x,\theta_t,\theta')$ and sample variance $\sigma^2(\Delta^*)$\;
    \While {$\sigma^2(\Delta^*) \ge 1$} {
    	Draw $m$ more samples to augment the current minibatch, and update $\Delta^*$ and $\sigma^2(\Delta^*)$ \;
    }
    Let $\sigma_n^2 = 1 - \sigma^2(\Delta^*)$, draw a normal variable $X_n \in \mathcal{N}(0,\sigma_n^2)$ and
...
$X_n$ and $X_c$. From this it follows that the acceptance test has the same error bound.
...
To apply this to our test, let $\mu = \frac{1}{N}\sum_{i=1}^N\log\frac{p(x_i|\theta')}{p(x_i|\theta)}$ which is
the expected log data probability ratio across the entire dataset. Then
Let $X_i = N(\log\frac{p(x_i|\theta')}{p(x_i|\theta)}-\mu)$, are zero-mean, IID variables.
Take $n=b$ the minibatch size, and then:
\begin{equation}
  \Delta^*(x,\theta,\theta') = \hat{X} + \Delta(x,\theta,\theta')
\end{equation}
...
$|X_i - \hat{X}|$ and $|X_i - \hat{X}|^3$ respectively on each minibatch. We expect this
...
\begin{corollary}
  If $|{\rm Pr}(\Delta^* < y) - \Phi(\frac{y-\Delta}{\sigma(\Delta^*)})| \leq \epsilon(\theta,\theta',b)$,
  then
  \begin{equation}
    |{\rm Pr}(\Delta^*+X_n+X_c < y) - S(y-\Delta)| \leq \epsilon(\theta,\theta',b)
  \end{equation}
  where $S(x)$ is the standard logistic function, and $X_n$ and $X_c$ are generated as per Algorithm 1. 
  \end{corollary}
...
\begin{proof}
  ...
  \begin{equation}
    P'(y) = Pr(\Delta^*+X_n < y) \qquad {\rm and}\qquad Q'(y) = \Phi\left({y-\Delta}\right)
  \end{equation}
  Now we convolve with the distribution of $X_c$ which gives:
  \begin{equation}
    P''(y) = Pr(\Delta^*+X_n+X_c < y) \qquad {\rm and}\qquad Q''(y) = S\left({y-\Delta}\right)
  \end{equation}
  Both steps preserve the error bound $\epsilon(\theta,\theta',b)$
\end{proof}
