...
\begin{equation}\label{eq:log_prob_poissons}
    \log p(\Theta) \approx \sum_{j=1}^{N_{\rm bins}} c_j \log (n P_j) - n P_j - \log(\Gamma(c_j+1)).
\end{equation}
...
on the term $C_{\theta,\theta'}$ over the entire the data set at every iteration
in order to calculate the error bound (in Equation~\ref{eq:bad_bound} of this
paper, i.e., Equation 9 of~\cite{icml2014c1_bardenet14}).  This bound must be
either computed analytically using properties of the specific dataset, or it
must be checked at each sample if a black-box test is required.  We found that
the sample code provided by the authors of~\cite{icml2014c1_bardenet14} in fact
computed this bound explicitly for each sample generated, i.e. the
implementation traversed the entire dataset for each sample, providing no
performance advantage over the complete test. For this reason, we omitted the
time to compute $C_{\theta,\theta'}$ from our measurements of the running time
of the sample implementation of \cite{icml2014c1_bardenet14}, and consider only
the time to process the minibatches.
...
\begin{table}[t]
    \caption{{\color{blue} Daniel: TODO Poisson Probabilities, also we want
    ground truth right?}}
    \label{tab:poissons}
    \centering
    \begin{tabular}{l l l l l}
    \toprule
    Model & Ground Truth & Ours & Korattikara 2014 & Bardenet 2014 \\
    \midrule
    Poission Value & TODO & TODO & TODO & TODO \\
    \bottomrule
    \end{tabular}
\end{table}
...
processed.  We see that our minibatch MH test is more efficient; it has similar
or better performance while consuming fewer data points.
