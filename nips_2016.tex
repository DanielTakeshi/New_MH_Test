...
$\theta_1, \ldots, \theta_T$ for large $T$ and approximates $p$ using the samples.  Each
...
data and tests the hypothesis that the sample $\theta'$ should be
accepted or rejected based on a probability ratio involving the
additional random variable $u\in [0,1]$. If the test cannot make a
decision with sufficient confidence, then the minibatch size is
increased and the test repeats. This process continues until a
decision. The bounds depend on either a central limit theorem
\cite{cutting_mh_2014} or a concentration bound
\cite{icml2014c1_bardenet14} requiring direct bounds on the log
likelihood ratio.  The problem with both methods is the expense of
resolving small differences.  In the worst case, all $N$ data points
may be needed, and as we show next this worst case can occur
about $\sqrt{N}$ times during the performance of $N$ tests.

We next prove that for some simple distributions, the lower
bound of average number of data instance consumed for one iteration of
~\cite{cutting_mh_2014,icml2014c1_bardenet14} is
$\Omega(\sqrt{N})$, where $N$ is the number of data points, see
Lemma~\ref{lem:efficienty_cutting}.

\begin{lemma}\label{lem:efficienty_cutting}
If the sample mean $\bar{l}$ has uniform distribution on $[-L,L]$,
the lower bound of expected data consumed of adaptive sampling method
for one iteration is $\frac{\eta s_{l}Q}{2L}\sqrt{N}$, where $\bar{l}
= \frac{1}{n}\sum_{i=1}^{n} \left (\log p(x_i; \theta') - \log p(x_i|
\theta_t)\right )$, $\eta = \phi^{-1}_{n-1}(1-\varepsilon)$,
$Q=\frac{p(\theta_t)q(\theta'|\theta_t)}{p(\theta')q(\theta_t|\theta')}$,
$s_l$ is the population standard deviation of $l_i$, $\phi_{n-1}()$ is
the CDF of t distribution with degree of freedom $n-1$, and $n$ is the
minibatch size.
\end{lemma}
...
\For{$t=\{1, \ldots, T\}$}{
    ...
    \While {${\rm std}(\Delta') \ge 1.2$} {
    	Draw $m$ more samples to augment the current minibatch, and update $\Delta'$ and ${\rm std}(\Delta')$ \;
    }
    Let $\sigma = \sqrt{1.2 - {\rm std}(\Delta')^2}$, draw a normal variable $X_n \in Norm(0,\sigma)$ and
