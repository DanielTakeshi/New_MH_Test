...
approximate version of the test $u < \alpha(\theta_t,\theta')$. If the
...
Following ~\cite{icml2014c1_bardenet14}, we write the test $u < \alpha(\theta_t,\theta')$ in
the equivalent form $\Lambda(\theta_t,\theta') > \psi(u,\theta_t,\theta')$ where
\begin{equation}
\Lambda(\theta_t,\theta') = \frac{1}{N}\sum_{i=1}^N \log\left(\frac{p(x_i|\theta')}{p(x_i|\theta_t)}\right)  
\hspace{0.2in} {\rm and} \hspace{0.2in}
\psi(u,\theta_t,\theta') = \frac{1}{N}\log\left(u\frac{q(\theta'|\theta_t)p(\theta_t)}{q(\theta_t|\theta')p(\theta')}\right)
\end{equation}

By the central limit theorem, we expect $\Lambda(\theta_t,\theta')$ to be approximately
Normal. To reduce computational effort, an unbiased estimate of $\Lambda(\theta_t,\theta')$
based on a minibatch can be used:

\begin{equation}
\Lambda^*(\theta_t,\theta') = \frac{1}{b}\sum_{i=1}^b \log\left(\frac{p(x_i|\theta')}{p(x_i|\theta_t)}\right)  
\end{equation}

The acceptance test then becomes a statistical test of the hypothesis
that $\Lambda(\theta_t,\theta') > \psi(u,\theta_t,\theta')$ by
establishing that $\Lambda^*(\theta_t,\theta')$ is substantially larger
than $\psi(u,\theta_t,\theta')$.  In ~\cite{cutting_mh_2014} an
...
\begin{equation}
\Lambda^*(\theta_t,\theta') =  \frac{1}{b}\sum_{i=1}^b \log\frac{p(x_i|\theta')}{p(x_i|\theta_t)}=
...
$\mathcal{N}(\theta'-\theta_t,1/N)$, so we take $q(\theta'\mid\theta_t) =
\phi((\theta'-\theta_t)N)$, where $\phi(x)$ is the Normal density
...
\begin{proof} Assume $(\theta' - \theta_t) \in \pm\frac{1}{\sqrt{N}}[0.5,1]$ and
  ...
  and guarantee that $\Lambda^*(\theta',\theta_t)$ is negative, which guarantees that we can find
  a $u$ so that $\psi(u,\theta',\theta_t)$ equals the expected value of $\Lambda^*(\theta',\theta_t)$.
  Specifically, choose $u_0$
  to satisfy
  \begin{equation}
    \log u_0 = N(\Lambda^*(\theta',\theta_t)-\psi(1,\theta',\theta_t))
  \end{equation}
  which evaluates to 
  \begin{equation}
  ...
  \begin{equation}
    \frac{1}{b}\sum_{i=1}^b x_i-\left(\theta_t+\frac{\theta'-\theta_t}{2} + \frac{\log u_0}{N(\theta'-\theta)}\right)
...
and consider the range of $u$ values
  ...
  The samples $\theta_t$, $(\theta'-\theta_t)$
...
\begin{equation}\label{eq:delta0}
    \Delta(x,\theta_t,\theta') =
\log \frac{p(\theta')\prod_{i=1}^N p(x_i \mid \theta')q(\theta_t \mid
  \theta')}{p(\theta_t)\prod_{i=1}^N p(x_i \mid \theta_t)q(\theta' \mid\theta_t)}
  \end{equation}
and we can separate out terms dependent and independent of the data $x$ as:
\begin{equation}\label{eq:delta1}
\Delta(x,\theta_t,\theta') = N\psi(1,\theta_t,\theta') + \sum_{i=1}^N\log\frac{p(x_i\mid\theta')}{p(x_i\mid\theta_t)}
\end{equation}
\begin{equation}\label{eq:delta2}
    \Delta^*(x,\theta_t,\theta') =
N\psi(1,\theta_t,\theta') + \frac{N}{b}\sum_{i=1}^b\log\frac{p(x_i\mid\theta')}{p(x_i\mid\theta_t)}
\end{equation}
Note that $\Delta$ and $\Delta^*$ are evaluated on the full dataset and a minibatch of size $b$
respectively. The scaling term $N$ produces true log likelihood ratios whereas $\Lambda-\psi$
is a scaled log ratio. $\Delta^*$ is an unbiased estimator of $\Delta$.

\subsection{A Full Data Acceptance Test}\label{ssec:deltas_full}

The key to our test is to smooth the acceptance function.  We consider tests other than the Metropolis
...
$p(\theta_y \mid \theta_x)$ is the probability of jumping from $\theta_x$ to $\theta_y$ in the
chain.
\end{lemma}

\begin{proof}
First note that the transition probability $p(\theta' \mid \theta_t)$ is the probability of proposing
...
e^\Delta \} = \min\left\{1, \frac{f(\theta')q(\theta_t \mid \theta')}{f(\theta_t)q(\theta' \mid
  \theta_t)}\right\}$ satisfies the condition $g(\Delta) = \exp(\Delta)g(-\Delta)$. In fact it is
the optimal acceptance function in terms of acceptance rate, since it accepts with probability
1 for $\Delta > 0$. 

\subsubsection{Logistic Acceptance Function}\label{ssec:deltas_full}
For our new MH test we use the logistic function: $g(\Delta) =
