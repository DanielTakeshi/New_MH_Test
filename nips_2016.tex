...
\cite{Barker65} rather than a Metropolis test, which makes our test
naturally error tolerant. The idea of using a noise-tolerant test
using Barker's test function was suggested but not explored
...
{\color{blue} Daniel: cleaned up this section, deleted redundant delta equation,
deleted a duplicate equation in Lemma~\ref{lem:detailed_balance}.}
...
$\theta'$. Let $V$ be a uniform random variable $V \sim \mathcal{U}(0,1)$. We
...
normal distribution by the central limit theorem. Later we will provide specific
bounds between the CDF of $\Delta^*(\theta,\theta')$ and a Normal CDF, but for
now we use an asymptotic argument. 
...
random $X$ (call it $X_{\rm log}$ from now on). In \cite{Bardenet15} it was
proposed to use $\Delta^*$ in a Barker test anyway and tolerate the fixed error
...
Our goal is to compute the distribution of the correction variable $X_{\rm corr}$ such that
$X_{\rm norm} + X_{\rm corr} = X_{\rm log}$, where $X_{\rm norm} \in \mathcal{N}(0,\sigma)$
and $X_{\rm log}$ has a standard logistic CDF $(1+\exp(-X))^{-1}$.
In the next section we show that the accuracy of the test depends on the absolute
error between the CDFs of $X_{\rm norm} + X_{\rm corr}$ and the distribution of $X_{\rm log}$.
So we minimize this in our construction.
...
\For{$t=\{1, \ldots, T\}$}{
    ...
    Draw $X_{\rm norm} \in \mathcal{N}(0,1-\sigma^2(\Delta*))$ and
...
{\color{blue} Daniel: I can help improve the table and describe it. Also, I'm
not sure right now about where Algorithm~\ref{alg:our_algorithm} should be
placed in the paper. Still TODO for me: clean up and/or revise the optimization
...
error of at most single-precision floating point error, and possibly better. We
therefore treat the $X_{\rm corr}$ variable as a sample from the exact
correction distribution and we will not analyze its errors.
...
We have shown that the distribution of $\Delta^*$ approximates a Normal
distribution with mean $\Delta$ and variance $\sigma^2(\Delta^*)$, and bounded
its error with estimable quantities. Both $E|X|$ and $E|X|^3$ can be estimated
as means of $|\Lambda_i - \Lambda|$ and $|\Lambda_i - \Lambda|^3$ respectively
on each minibatch. We expect this will often be accurate enough on minibatches
with hundreds or thousands of points, but otherwise bootstrap CIs can be
computed from those sequences. Since the bounds are monotone in $E|X|$ and
$E|X|^3$, using upper bootstrap CI limits will provide high-confidence error
bounds.

{\color{blue} Daniel: TODO provide a better transition for this paragraph.}
...
Equation 2.2 in Bentkus et al.'s work on Edgeworth expansions~\cite{Bentkus97}
...
%In %Section~\ref{ssec:nets}, we apply the test for deep learning. 
Appendices~\ref{app:gaussian},~\ref{app:logistic} %and~\ref{app:nnet} 
...
%full-batch MCMC (``standard MCMC'') with the standard test from Equation~\ref{eq:traditional}, 
...
%Usually, the method can make a decision with 200 samples. Occasionally, however, it must use all 10000 points, resulting in lots of wasted computation.  For this particular experiment, that happened eight times.
...
%The third plot of Figure~\ref{fig:logistic_minibatch} shows performance based on minibatch size. The
...
%Similar as the Subsection~\ref{ssec:gaussians}, we use Figure~\ref{fig:exp_3}
%to reveal the accuracy of our estimation of ${\rm std}(\Delta')$ and stability
%of $\rho/\sigma^3$.
...
\begin{itemize}[noitemsep]
    ...
    %\item More information on Section~\ref{ssec:nets}. % Daniel: this is in
    %progress.
\end{itemize}
