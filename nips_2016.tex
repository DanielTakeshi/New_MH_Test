...
approximate by sampling $\theta$ values. When $n$ is large (e.g., in
the billions), it is computationally difficult to compute the exact
posterior because the likelihood must be evaluated on each $x_i$.
