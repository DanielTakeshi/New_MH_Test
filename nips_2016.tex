...
a heated Gaussian mixture model (additional details are in
Appendix~\ref{app:gaussian}). In Section~\ref{ssec:logistic}, we analyze its
...
$\theta_i$ that fall in bin $j$ out of $N_{\rm bins}$ total. We omit bins in
which either the source or MCMC-generated distributions did not have a single
sample, resulting in the set of (at most $N_{\rm bins}$) bins $B$. Then the log
probability for the set of samples is approximated as
\begin{equation}\label{eq:log_prob_poissons}
    ...
    \log p(\Theta) \approx \sum_{j \in B} c_j \log (n P_j) - n P_j - \log(\Gamma(c_j+1)).
\end{equation}
...
\in \pm\frac{1}{\sqrt{N}}[0.5,1]$ with matching sign (i.e., first establish
$(\theta'-\theta)$, then ``force'' $\theta-0.5$ to be the same sign).  As
...
\Phi(\sqrt{N}\frac{1}{\sqrt{N}}) - \Phi(\sqrt{N}\frac{0.5}{\sqrt{N}}) = \Phi(1)
- \Phi(0.5) \approx 0.14988$. 

These events together guarantee that $\Lambda^*(\theta,\theta')$ is negative,
intuitively because the next choice of $\theta'$ must be ``further away'' from
0.5 than $\theta$.  This fact means that we can find a $u \in (0,1)$ so that
...
\psi(u,\theta,\theta')$ required by these methods, where $\not\approx$
informally means ``significantly different from'' under the distribution over
...
\end{equation}
where, again following~\cite{langevin_2011}, we set $\sigma_x^2 = 2$ and we draw
the data with $\theta_1=0$ and $\theta_2=1$ fixed (though these values are
arbitrary, and different choices would yield different posterior shapes).
This means the log likelihood of a single data instance is
\begin{equation}\label{eq:log_ll_single_instance}
...
in Section~\ref{ssec:logistic}.} of $\theta$, such as the MAP estimate. (If $N$
is extremely large, it will cause the posterior to peak sharply at its modes,
reducing distribution estimates to point estimates.)
...
% Daniel: This was some stuff from the Logistic Regression appendix, but we no
% longer need that.

%% In the logistic regression experiment of Section~\ref{ssec:logistic}, the
%% feature vector for an image consists of its pixel values, normalized between 0
%% and 1. For simplicity, we consider the binary classification case, so we only
%% use digits one ($y = 1$) and seven ($y = -1$).  The probability of the $i^{\rm
%% th}$ output $y_i \in \{-1,+1\}$ with the input vector $x_i$ is
%% \begin{equation}\label{eq:logistic_probability}
