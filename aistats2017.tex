...
\begin{equation}\label{eq:conv2}
    \scriptsize{C_\sigma^* = \argmin_{C_\sigma} \max_{i \in \{-2N,\ldots,2N\}}\left|\sum_{j = -N}^{N}\Phi_{\sigma}(X_i-Y_j) C_{\sigma}(Y_j) - S(X_i)\right|.}
\end{equation}
...
\scriptsize{-6.4E|X|^3-2E|X| \leq    \sup_x|{\rm Pr}(t<x) - \Phi(x)|{\sqrt{n}} \leq 1.36E|X|^3}
\end{equation}
...
\begin{equation}
\begin{split}
\tiny{-\epsilon} & = \tiny{\int_{-\infty}^{+\infty} -\epsilon R(x) dx } \\
& \tiny{\leq \int_{-\infty}^{+\infty}(P(z-x)-Q(z-x))R(x) dx }\\
& \tiny{\leq \int_{-\infty}^{+\infty}\epsilon R(x) dx = \epsilon}\\
\end{split}
\end{equation}
\end{proof}
...
\begin{table}[h]
    ...
    \begin{tabular}{l l l l}
    %\toprule
    {\bf Model} & {\bf Our Method} & {\bf Korattikara 2014} & {\bf Bardenet 2014} \\
    %\midrule
    \hline \\
...
%% \end{equation}
%% The problem statement is as follows: given some number of conditionally
