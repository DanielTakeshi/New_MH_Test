...
\begin{abstract}
  ...
  We
  present a novel Metropolis-Hastings method for large datasets that
  ...
  correction variable.  The resulting test has simlar cost to a normal
  SGD update. Our experiments demonstrate several order-of-magnitude
  speedup over previous work.
\end{abstract}
...
our test to be applied. Theses samples can cover the search space densely with
...
%1$.
%In fact, $g(s) =\exp(s) \wedge 1$ is the optimal acceptance function in
...
error between logistic and normal distributions. 
...
% Daniel: I moved the previous Section 5.4 part to the supplementary material
% since our code doesn't do this, so we should not describe this in much detail.
...
equal to the overall test error. Thus our results for
~\citet{cutting_mh_2014} should be treated as lower bounds. All
methods were initialized with minibatch size 100.  For ~\citet{cutting_mh_2014} we
increment sizes by 100 and set the tolerance $\epsilon=0.005$.  For
~\citet{icml2014c1_bardenet14}, we increase sizes geometrically with $\gamma = 1.5$ and
...
%%% The pixels are scaled in $[0,1]$  - JFC Why ????
...
set at $T=1000$. We run the three tests for samples and again use 100
...
symbolic bound for $C_{\theta,\theta'}$ for Logistic regression from
~\citet{icml2014c1_bardenet14}, but found it to be too high. Instead
we used the empirical $C_{\theta,\theta'}$ from the entire dataset
without including the test for this test, so again our results
represent a lower bound for this method.
...
\item Some reviewers mentioned that our deconvolution approach to
  ...
  ill-defined.  We added detail description on how the correction
  distribution is computed. While the distribution is not unique, we
