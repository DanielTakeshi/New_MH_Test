...
in practice, it yields an acceptable $L_{\infty}$ norm.
...
Here, we use quantitative forms of the Central Limit Theorem which rely on
measurable statistics from a single minibatch. Thus a sampler using our approach
does not need to see data beyond the current minibatch, and can easily be
applied on large datasets and in online modes where the data is presented as a
stream.
...
For~\citet{cutting_mh_2014}, we increment minibatches by 100 and
for~\citet{icml2014c1_bardenet14}, we increase the minibatch size geometrically
with a ratio of $\gamma = 1.5$. These methods have different approaches to
repeated testing: the former uses an aggressive testing approach, explicitly
modeling correlation between tests and allowing many more tests for a given
$\delta$, while the latter uses discounting, allocating a fraction of the test
error $\delta$ to each minibatch test.
...
sample falling into bin $i$ (denoted as $P_i$) is the integral of the true
posterior probability over the area of that bin.  A single sample from any of
the MH methods should therefore be multinomial with distribution $P$, and a set
of $n$ (ideally independent) samples should follow a ${\rm Multinomial}(P,n)$
distribution.  Since the ideal distribution is simple, we can use it to measure
the quality of the sample distributions rather than use general purpose tests
like KL-divergence or likelihood-ratio.  The latter are problematic with zero
